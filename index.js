...
//Função questão aleatória
function questRamdom (quest) {return quest.sort( ()=> Math.random() - 0.5)};

//função seleciona as 10 primeiras perguntas da array de perguntas aleatórias
function questionsSelected (quest) {return quest.slice(0,10)};

//função que exibe as perguntas
//function showQuestions (quest) {return quest.forEach((pergunta, index) => console.log(`${index +1}  ${pergunta.pergunta}`))};

//aplicação das funções
let perguntasAleatorias = questRamdom(questoes);
let perguntasSelecionadas = questionsSelected(perguntasAleatorias);
//let exibirQuestoes = showQuestions(perguntasSelecionadas);
let acertos = 0;
let erros = 0;


//função exibe e valida as perguntas
function teste (dados,index) {
  for (let i=0; i<10; i++){
    console.log(`\n${i +1}:  ${dados[i].pergunta}`);
    let resposta = input.question("Digite a resposta (ano): ");

    if (resposta == dados[i].resposta){
...
teste (perguntasSelecionadas,0);

let mensagemFinal; 

if (acertos <= 3) {
  mensagemFinal = "OH não! Tente mais uma vez!"} 
else if (acertos > 3 && acertos < 7 ) {
